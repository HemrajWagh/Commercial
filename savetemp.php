...
$siteUrl='https://kumarprimus.kumarworld.com/';
...
{ //shubham.patni@kumarworld.com, roshan.anand@kumarworld.com, atul.rathod@kumarworld.com, sanjeev.deshpande@megapolis.co.in, kumar.saurabh@megapolis.co.in, pallavi.shendge@megapolis.co.in
  ...
  $username = "Kp_Primus";
  $password = "qdltThkbm2eTIg3F";

  try {
    $conn = new PDO("mysql:host=$servername;dbname=Kp_Primus_db", $username, $password);
  ...
  $query = "INSERT INTO conform SET name=:name, email=:email, mobile=:phone, comment=:message, project=:pid, source=:source , utm_source=:utm_source, utm_medium=:utm_medium , utm_campaign=:utm_campaign";
  ...
  $message = htmlspecialchars(strip_tags($message));
  $project = htmlspecialchars(strip_tags($project));
  $UtmSource = htmlspecialchars(strip_tags($utm_source));
  $UtmMedium = htmlspecialchars(strip_tags($utm_medium));
  $UtmCampaign = htmlspecialchars(strip_tags($utm_campaign));
  // $campaign = "KP_Digital";
  $source = "KP_WebSite";
  $stmt->bindParam(":name", $name);
  $stmt->bindParam(":email", $email);
  ...
  // $countricode=$_REQUEST['countryCode'];
   ...
   $headers = "From: Kumar-Primus@kumarprimus.kumarworld.com \r\n";
